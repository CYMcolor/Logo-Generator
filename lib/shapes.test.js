...
//test variables to be passed to the parmeters
...
describe('Test Shapes:', () =>{
    ...
    describe('Render SVG file:', () => {
        ...
        it('Should return true when the results of setText() and setColor() can be passed to make SVG file', () => {
            const circle = new Circle(text, textColor, 'circle', shapeColor);
            const textString = circle.setText();
            const shapeString = circle.setColor();
            const result = circle.makeSVG(textString,shapeString);
            const svg = `<svg version="1.1" width="300" height="200" xmlns="http://www.w3.org/2000/svg">\n\t${shapeString}\n\t${textString}\n</svg>`
